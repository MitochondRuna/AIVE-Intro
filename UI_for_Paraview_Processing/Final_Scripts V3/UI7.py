...
def save_config():
    ...
    config = {
        ...
        "export_x3d": export_x3d_var.get(),
        "export_stl": export_stl_var.get()
    }
...
def load_existing_config_file():
    ...
    # Set export options if available in the config
    export_x3d_var.set(config.get('export_x3d', False))
    export_stl_var.set(config.get('export_stl', False))
...
export_x3d_var = tk.BooleanVar()
export_stl_var = tk.BooleanVar()

export_x3d_check = tk.Checkbutton(root, text="Export .x3d", variable=export_x3d_var)
export_x3d_check.place(x=240, y=20 + row_spacing * 4)

export_stl_check = tk.Checkbutton(root, text="Export .stl", variable=export_stl_var)
export_stl_check.place(x=360, y=20 + row_spacing * 4)

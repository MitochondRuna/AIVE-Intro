...
//    }
    ...
    private static void processFile(File file, int taskNumber, File outputDir) throws Exception {
        ...
        AttributeSelection filter = new AttributeSelection();  // 这不是一个过滤器，而是一个属性选择器
        InfoGainAttributeEval eval = new InfoGainAttributeEval();
        Ranker search = new Ranker();

        // 设置Ranker的阈值，只保留信息增益大于0.5的属性
        ...
        // 应用属性选择
        filter.SelectAttributes(dataset);  // 使用SelectAttributes方法进行属性选择

        Instances newData = filter.reduceDimensionality(dataset); // 根据选择的属性减少数据集的维度
